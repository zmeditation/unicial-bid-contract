...
Bid contract for ERC721 tokens. Forked from DCL bid-contract

## How it works

- Bidders should approve the Bid Contract to operate UCC on their behalf.
...
- If the token owner wants to accept a bid, he should transfer the token to the Bid Contract using `safeTransferFrom(owner, bid_contract, tokenId, bidId)`.
  Once the Bid Contract receives the token (onERC721Received) it will check if the bid is valid and will transfer the UCC from the bidder to the token owner
...
- If the token has an active publication in the Unicial Marketplace when a bid is accepted, the order will become invalid because the owner changed.
...
contract Bid is Ownable {
    ...
    * @param _UCCToken - address of the UCC token
    * @param _owner - address of the owner for the contract
    */
    constructor(address _UCCToken, address _owner) Ownable() Pausable() public;
